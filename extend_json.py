...
def create_mock_irt(course, strand, text):
    ...
    tags = []
    low = text.lower()
    if 'write' in low or 'written' in low or 'paragraph' in low or 'essay' in low: tags.append('writing')
    if 'read' in low or 'text' in low or 'source' in low or 'analyse' in low: tags.append('reading')
    if 'solve' in low or 'calculate' in low or 'equation' in low or 'graph' in low: tags.append('math')
    if 'eqao' in low or 'provincial' in low or 'standardized' in low: tags.append('eqao')
    if 'write' in low or 'note' in low or 'diagram' in low or 'label' in low: tags.append('stylus')

    return {'irt_b': round(b, 2), 'irt_a': round(a, 2), 'irt_c': 0.2, 'tags': tags}


def process_course_data(course_id, name, url, raw_strands):
    course_data = {'name': name, 'official_url': url, 'strands': {}}
