...
void main() {
  ...
  test('learning evidence contract does not pretend Mesh event admission', () {
    final contract = _contract('contracts/axiom-education-learning-evidence.v1.json');
    final boundary = contract['mesh_admission_boundary'] as Map<String, dynamic>;
  ...
  test('collaboration contract keeps transcripts need-to-know', () {
    final contract =
        _contract('contracts/axiom-education-collaboration-privacy.v1.json');
  ...
  test('entry assessment does not convert missing hardware into learner failure', () {
    final contract = _contract('contracts/axiom-education-entry-assessment.v1.json');
    final selection = contract['selection_invariants'] as Map<String, dynamic>;
    ...
    expect(selection['missing_modality_is_not_negative_ability_evidence'], isTrue);
    expect(evidence['device_failure_is_not_learner_failure'], isTrue);
  });

  test('model routing contract hard-filters privacy before quality and cost ranking', () {
    final contract = _contract('contracts/axiom-education-model-routing.v1.json');
    ...
    final authority = contract['education_authority_boundary'] as Map<String, dynamic>;

    expect(dependency['model_output_is_data_not_authority'], isTrue);
    expect(context['full_longitudinal_learner_profile_sent_by_default'], isFalse);
    expect(context['only_explicitly_granted_context_scopes_may_be_materialized'], isTrue);
    expect(authority['model_may_create_grade_or_credit'], isFalse);
    expect(authority['model_may_expand_its_own_context_scope'], isFalse);
    expect(authority['consequential_assessment_requires_separate_evidence_and_verification'], isTrue);
  });
}
