...
void main() {
  ...
  group('Database initialization and providers', () {
    test(
      'copies and verifies the bundled database before serving providers',
      () async {
        final outputDirectory = Directory.systemTemp.createTempSync(
          'ontarioedai-output-',
        );
        final assetDirectory = Directory.systemTemp.createTempSync(
          'ontarioedai-asset-',
        );
        ...
        final detail = await container.read(
          courseDetailProvider('MTH1W').future,
        );
        expect(detail.strands.single.expectations.length, 2);
        expect(
          detail.strands.single.expectations.last.tags,
          contains('advanced'),
        );

        final unknown = await container.read(
          courseDetailProvider('UNKNOWN').future,
        );
    ...
    test(
      'fails closed when the database directory cannot be created',
      () async {
