...
void main() {
  group('Verification Tests', () {
    ...
    test('returns false and prints message when data is null', () {
      ...
      expect(printLogs, contains('Verification Failed: Data is empty or null.'));
    });
    ...
    test('returns false and prints message when missing required fields', () {
      ...
      expect(printLogs, contains('Verification Failed: Missing required fields (id, timestamp).'));

      printLogs.clear();
      final result2 = runWithPrintInterception({'timestamp': 1000}); // missing id
      expect(result2, isFalse);
      expect(printLogs, contains('Verification Failed: Missing required fields (id, timestamp).'));
    });

    test('returns false and prints message when timestamp is invalid format', () {
      final result = runWithPrintInterception({
        'id': 1,
        'timestamp': 'not an int',
      });
      expect(result, isFalse);
      expect(printLogs, contains('Verification Failed: Invalid timestamp format.'));
    });

    test('returns false and prints message when timestamp is in the future', () {
      ...
      expect(result, isFalse);
      expect(printLogs, contains('Verification Failed: Timestamp is in the future.'));
    });
    ...
    test('returns true and prints message when data is valid', () {
      ...
      expect(printLogs, contains('Verification Passed: Data integrity check successful.'));
    });
  });
}
