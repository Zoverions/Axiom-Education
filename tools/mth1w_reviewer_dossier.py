...
content-addressed lesson and accessibility review targets, source-use inventory,
cumulative assessment evidence, accessible/offline lesson exports, current readiness
blockers, deliberately non-valid reviewer templates, and any already-submitted review
provenance into a reproducible directory.
"""
...
ACCESSIBILITY_REVIEW_DIR = ROOT / "curriculum" / "reviews" / "mth1w-accessibility"
LICENCE_REVIEW_DIR = ROOT / "curriculum" / "licensing" / "mth1w" / "reviews"
EXPECTED_LESSONS = 43
EXPECTED_ACCESSIBILITY_TARGETS = 47
...
from tools.mth1w_accessibility_review_evidence import (  # noqa: E402
    build_plan as build_accessibility_review_plan,
    verify_directory as verify_accessibility_reviews,
)
from tools.mth1w_accessibility_review_templates import (  # noqa: E402
    build as build_accessibility_templates,
)
...
def build_review_guide(
    ...
    accessibility_plan: dict[str, Any],
    ...
    lines = [
        ...
        f"- Accessibility/usability targets: **{accessibility_plan['target_count']}** = **{accessibility_plan['lesson_alternative_targets']}** lesson alternatives + **{accessibility_plan['application_surface_targets']}** platform-specific learner-app surfaces",
        f"- Accessibility platform targets: **{', '.join(accessibility_plan['application_platforms'])}**",
        f"- Submitted accessibility review records: **{accessibility_summary['reviews']}**",
        f"- Currently approved accessibility targets: **{accessibility_summary['latest_approved_targets']}/{accessibility_summary['targets']}**",
        ...
        "4. For accessibility/usability work, use `accessibility-review-plan.json` and the deliberately non-valid files under `accessibility-review-templates/`. Lesson alternatives and Android, Windows, macOS, and iOS app surfaces are separate targets.",
        "5. Record instructional/cultural findings against the exact lesson target digest using the lesson review schema. Record accessibility findings against the exact accessibility target digest and platform, where present. Do not edit a review to point at a new digest or platform after content changes.",
        "6. Use `source-use-inventory.json` for licensing review. A licensing decision must bind to the exact `source_use_sha256`; public availability alone is not redistribution permission.",
        "7. Review cumulative assessment design separately. Lesson approval does not establish assessment validity, mastery, grade, or credit.",
        "8. Preserve negative findings (`changes-required`, `rejected`, restricted/unresolved licensing decisions) as provenance rather than deleting them.",
        "",
        "## Submission commands",
        "",
        ...
        "```",
        "",
        "Accessibility/usability review evidence:",
        "",
        "```bash",
        "python tools/mth1w_accessibility_review_evidence.py verify-review path/to/accessibility-review.json",
        "```",
        "",
        ...
        "## Non-claims",
        "",
        "This dossier does not make Axiom Education a school, grant Ontario course credit, establish Ministry approval, prove learner mastery, decide copyright law, establish WCAG/AODA conformance, prove assistive-technology usability on an unreviewed platform, or establish App Store/signed-release/production support. It is a reproducible evidence package for qualified human review.",
    ]
    return ("\n".join(lines).rstrip() + "\n").encode("utf-8")
...
def build(output: Path) -> dict[str, Any]:
    ...
    accessibility_plan = build_accessibility_review_plan()
    source_inventory = build_inventory()
    lesson_summary = verify_lesson_reviews()
    accessibility_summary = verify_accessibility_reviews()
    ...
    require(
        accessibility_plan.get("target_count") == EXPECTED_ACCESSIBILITY_TARGETS,
        "accessibility review plan does not contain 47 current targets",
    )
    ...
    write_json(output / "accessibility-review-plan.json", accessibility_plan)
    ...
    write_json(
        output / "submitted-review-summary.json",
        {
            ...
            "accessibility_reviews": accessibility_summary,
    ...
    accessibility_templates_dir = output / "accessibility-review-templates"
    accessibility_templates_manifest = build_accessibility_templates(
        accessibility_templates_dir
    )
    require(
        accessibility_templates_manifest.get("target_count")
        == EXPECTED_ACCESSIBILITY_TARGETS,
        "accessibility reviewer templates do not cover all 47 targets",
    )
    ...
    copied_accessibility_reviews = copy_verified_review_records(
        ACCESSIBILITY_REVIEW_DIR,
        output / "submitted-evidence" / "accessibility-reviews",
    )
    ...
    require(
        len(copied_accessibility_reviews) == accessibility_summary["reviews"],
        "accessibility review copy count mismatch",
    )
    ...
    manifest = {
        ...
        "accessibility_target_count": accessibility_plan["target_count"],
        "accessibility_application_platforms": accessibility_plan[
        ...
        "submitted_accessibility_review_count": accessibility_summary["reviews"],
        ...
        "accessibility_review_plan_sha256": file_sha256(
            output / "accessibility-review-plan.json"
        ),
        "source_use_inventory_sha256": file_sha256(output / "source-use-inventory.json"),
        "claim_boundary": (
            "This dossier is a deterministic packaging of current machine evidence, draft lesson alternatives, non-valid reviewer templates, and submitted human-review provenance. "
            "It creates no educator, cultural, licensing, accessibility, assessment, mastery, grade, credit, platform-production, or Ministry approval."
        ),
        "files": files,
    }
...
def verify_determinism() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="axiom-review-dossier-a-") as first_dir, tempfile.TemporaryDirectory(prefix="axiom-review-dossier-b-") as second_dir:
        ...
        require(first_manifest["lesson_target_count"] == EXPECTED_LESSONS, "reviewer dossier lesson target count drifted")
        require(
            first_manifest["accessibility_target_count"]
            == EXPECTED_ACCESSIBILITY_TARGETS,
            "reviewer dossier accessibility target count drifted",
        )
    ...
    try:
        if args.command == "build":
            ...
            print(
                ...
                f"{manifest['accessibility_target_count']} accessibility targets, "
