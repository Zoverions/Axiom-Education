...
class CurriculumSourceMonitoringTests(unittest.TestCase):
    ...
    def test_current_policy_accounts_for_sixteen_locks_and_zero_pending_targets(self) -> None:
        result = verify_policy()
        self.assertEqual(result["sources"], 16)
    ...
    def test_html_response_surface_cannot_be_promoted_to_strict_without_new_evidence(self) -> None:
        def mutate(payload):
            self.source(payload, "ontario-fr-francais-grades-1-8-2023").update(
    ...
    def test_every_committed_lock_must_remain_accounted_for(self) -> None:
        def remove_fr_arts(payload):
            payload["sources"] = [
                source
                for source in payload["sources"]
                if source["source_id"] != "ontario-fr-arts-grades-1-8-2009"
            ]

        path = self.mutation(remove_fr_arts)
    ...
    def test_committed_c1_lock_cannot_lose_its_capture_target(self) -> None:
        def remove_fr_arts(payload):
            payload["targets"] = [
                ...
                if target["source_id"] != "ontario-fr-arts-grades-1-8-2009"
            ]

        path = self.target_mutation(remove_fr_arts)
