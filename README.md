...
The branch also contains an experimental Axiom-authored cross-cutting computational-literacy competency family: 18 non-jurisdictional nodes spanning Intent, Specification, Verification, and Systems Understanding, with no competency edges in v1. The bounded executable proofs are one deterministic MTH1W verifier-selection experience and one optional governed-Socratic Claw verification-reasoning experience. This is **not official Ontario curriculum**, is **not validated psychometrics**, creates **no AI-literacy score**, has **no model requirement**, and has **no automatic mastery or grade effect**. Model output cannot establish correctness or learner evidence; `MathAnswerVerifier` remains the sole correctness authority for the bounded MTH1W answer-checking path, and both proofs retain non-generative/no-AI behavior.
...
- the Axiom-authored cross-cutting computational-literacy capability is experimental: exactly 18 non-jurisdictional nodes, zero v1 edges, one deterministic MTH1W proof, and one optional governed-Socratic Claw proof; it is not official Ontario curriculum, not validated psychometrics, produces no AI-literacy score, requires no model, and has no automatic mastery or grade effect;
